...
from schema import GroupField, MapField, CumulativeField, SnapshotField, AggregateField, TimeElapsedField
...
def read_data():
    ...
    All values here are absolute readings - the schema tree (see "Schema construction" below) turns cumulative counters into deltas, and
    gauges into snapshots/aggregates, per retention rule.
    """
...
# Schema construction ---
# See schema.py for the Field types themselves. build_schema turns one retention rule's mode into a
# tree of Field instances shaped like read_data()'s output - see doc/Monitoring.md's Monitor Format
def make_snapshotaggregate(mode):
    """
    Returns a snapshotaggregate(expand) macro resolved to mode: a gauge field that's a SnapshotField
    in "snapshot" mode, or an AggregateField(expand=expand) in "aggregate" mode. expand is dropped on
    the floor for SnapshotField - it's only ever meaningful for AggregateField.
    """
    def snapshotaggregate(expand=False):
        return SnapshotField() if mode == "snapshot" else AggregateField(expand=expand)
    return snapshotaggregate

def build_schema(mode):
    """
    Builds one retention rule's schema tree, matching the Monitor Format documented in doc/Monitoring.md.
    """
    snapshotaggregate = make_snapshotaggregate(mode)
    def mem_schema():
        return GroupField(total=SnapshotField(), used=snapshotaggregate(expand=True))
    return GroupField(
        actualPeriod=TimeElapsedField(),
        sys_cpu=GroupField(
            agg=GroupField(total=CumulativeField(), busy=CumulativeField()),
            ind=MapField(lambda: GroupField(total=CumulativeField(), busy=CumulativeField())),
        ),
        sys_mem=mem_schema(),
        sys_net_io=GroupField(
            agg=GroupField(sent=CumulativeField(), recieved=CumulativeField()),
            ind=MapField(lambda: GroupField(sent=CumulativeField(), recieved=CumulativeField())),
        ),
        sys_disk_io=GroupField(
            agg=GroupField(read=CumulativeField(), written=CumulativeField()),
            ind=MapField(lambda: GroupField(read=CumulativeField(), written=CumulativeField())),
        ),
        ...
        cgroups=MapField(lambda: GroupField(
            cpu=CumulativeField(),
            mem=mem_schema(),
            disk_io=GroupField(read=CumulativeField(), written=CumulativeField()),
            _fold=False
        )),
    )
...
def refresh_reset_detection(state: dict):
    ...
    retention rule's stored "data" in state["intervals"] (see "Cumulative Counters" and "State" in the
    documentation). This runs every mainloop tick, not just when a rule is due to write, so a slower rule can't
    miss a reset that a faster rule already saw. Read failures are logged (via safe_call) and skipped for this
    tick, rather than treated as a reset.
    """
    ...
    if boot_id is not None:
        if state["boot_id"] is not None and boot_id != state["boot_id"]:
            ...
            for interval_state in state["intervals"].values():
                if interval_state["data"] is not None:
                    interval_state["data"]["sys_cpu"] = None
                    interval_state["data"]["sys_net_io"] = None
                    interval_state["data"]["sys_disk_io"] = None
    ...
    for cgroup in CGROUPS:
        ...
        if cgroup in state["cgroup_inodes"] and inode != state["cgroup_inodes"][cgroup]:
            ...
            for interval_state in state["intervals"].values():
                if interval_state["data"] is not None:
                    interval_state["data"].get("cgroups", {}).pop(cgroup, None)
...
rule_schemas = {get_rule_key(interval, number): build_schema(mode) for interval, number, mode in RETENTION_RULES}  # One stateful schema tree per retention rule, lives for the process
previous_tick_time = None  # Precise time of the previous mainloop tick, used to weight this tick's contribution to AggregateField trackers
while True:
    ...
    # 0 for the very first ever tick (no previous tick to measure from)
    ...
    # Fold this tick into every rule's schema tree - unconditional, every rule, every tick (only
    # AggregateField nodes actually do anything with it)
    for schema in rule_schemas.values():
        schema.iterate(data, tick_dt)
    ...
    # Write each due rule's record - every rule finalises against (and then updates) its own baseline in state
    for path, rule_key in needs_new:
        rule_state = state["intervals"].setdefault(rule_key, {"time": None, "data": None})
        actual_period = None if rule_state["time"] is None else current_time_exact - rule_state["time"]
        record = rule_schemas[rule_key].finalise(data, rule_state["data"], actual_period)
        open(os.path.join(path, f"{current_time}.json"), "w").write(json.dumps(record))
        state["intervals"][rule_key] = {"time": current_time_exact, "data": data}
