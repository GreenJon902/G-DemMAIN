 ...
 */
export function sendWebloginWebhook(name) {
 ...
 * Note this will not wait for the webhook to finish.
 */
export function sendWebcommandWebhook(name, command) {
...
function executeCommand(...args) {
    if (!C().DONT_REQUIRE_WEBHOOKS_FILE && !existsSync(WEBHOOKS_FILE))
        throw "Webhooks file does not exist"; // Ensure that it exists
