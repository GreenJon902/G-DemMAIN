...
const generate = () => {
    ...
    // The dynmap output folder
    const DYNMAP_FOLDER = resolvePath(zTNe.parse(gWeb.dynmapFolder));
    const DYNMAP_MARKERS_FILE = path.join(DYNMAP_FOLDER, "markers.yml");
    const DYNMAP_WORLDS_FILE = path.join(DYNMAP_FOLDER, "worlds.txt");
    const DYNMAP_MARKER_ICONS_FOLDER = path.join(DYNMAP_FOLDER, "tiles", "_markers_");
    const DYNMAP_TILES_FOLDER = path.join(DYNMAP_FOLDER, "tiles", "world");
    ...
    return {
        MCCWSS_PORT, MCCWSS_TAIL, SESSION_PASSWORD, LIST_FOLDER, MC_LOG_FOLDER, MONITOR_FOLDER, MINECRAFT_CACHE_FILE, DYNMAP_MARKERS_FILE, DYNMAP_WORLDS_FILE, DYNMAP_MARKER_ICONS_FOLDER, DYNMAP_TILES_FOLDER, MINECRAFT_MONITOR_CONSOLE_PORT, MINECRAFT_MONITOR_CONSOLE_HOST, MINECRAFT_MONITOR_CONSOLE_AUTH_KEY, G_WEB_DATABASE_USER, G_WEB_DATABASE_PASSWORD, G_WEB_DATABASE_HOST, G_WEB_DATABASE_PORT, TRACKED_UNITS, MONITOR_RETENTION_RULES
    };
};
