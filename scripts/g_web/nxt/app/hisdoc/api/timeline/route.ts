...
export async function GET(req: NextRequest): Promise<NextResponse> {
    ...
    const events = await prisma().hisdoc_event.findMany({
        where,
    ...
    // JSON.stringify throws on BigInt — convert all FlexiDate bigint fields to Number
    const serialised = page.map(e => ({
        ...e,
