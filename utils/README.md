...
## Syncing Configs - `sync-config.py`
This module manages the contents and deployments of the synced config files.
This script should be executed in the `<repo_root>/config` folder.

### Deployment
The `sync-config.py` script will copy the of the contents of folders specified in `sync-map.ini` to their respective destinations (also specified by that file).
